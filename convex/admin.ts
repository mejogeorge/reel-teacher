...
/** Queue a rendered word's video to be published to one or more platforms. */
export const publishReel = mutation({
  args: { wordId: v.id("words"), platforms: v.optional(v.array(v.string())) },
  handler: async (ctx, { wordId, platforms }) => {
    await requireAdmin(ctx);
    const word = await ctx.db.get(wordId);
    if (!word) throw new Error("word not found");
    if (word.status !== "rendered") throw new Error("word must be rendered before publishing");
    ...
    const targets = platforms && platforms.length > 0 ? platforms : ["instagram", "facebook"];
    const now = Date.now();
    for (const platform of targets) {
      ...
      if (platform === "instagram") {
        ...
        await ctx.scheduler.runAfter(0, internal.publish.facebookPublish, { wordId, postTargetId: id });
      } else {
    ...
    await logEvent(ctx, {
      wordId,
      type: "publish.queued",
      message: `Queued publish: ${targets.join(", ")}`,
    });
  },
});
